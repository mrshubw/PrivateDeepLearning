...
work_information.set('固定其他超参数，画出测试集准确率随epsilon变化的曲线图')
...
# sigma按钮的命令函数
def sigma_constant():
    entry_sigma['state'] = 'normal'


def sigma_square():
    entry_sigma['state'] = 'disabled'
...
# 输入sigma
label_sigma = tkinter.Label(root, text='epsilon:')
label_sigma.grid(row=2, column=0, sticky='e')
text_sigma = tkinter.StringVar()
text_sigma.set('5')
entry_sigma = tkinter.Entry(root, textvariable=text_sigma, bd=3)
entry_sigma.grid(row=2, column=1, sticky='w')


# 输入clip_bound
label_clip = tkinter.Label(root, text='clip_bound:')
label_clip.grid(row=3, column=0, sticky='e')
text_clip = tkinter.StringVar()
text_clip.set('0.01')
entry_clip = tkinter.Entry(root, textvariable=text_clip, bd=3)
entry_clip.grid(row=3, column=1, sticky='w')

# 输入batch_size
label_batch = tkinter.Label(root, text='batch size:')
label_batch.grid(row=4, column=0, sticky='e')
text_batch = tkinter.IntVar()
spin_batch = tkinter.Spinbox(root, values=(100, 200, 300, 600, 1000), textvariable=text_batch)
text_batch.set(600)
spin_batch.grid(row=4, column=1, sticky='w')

# 输入num_steps
label_steps = tkinter.Label(root, text='num of steps:')
label_steps.grid(row=5, column=0, sticky='e')
text_steps = tkinter.IntVar()
text_steps.set(100)
spin_steps = tkinter.Spinbox(root, from_=100, to=160000, increment=100, textvariable=text_steps)
spin_steps.grid(row=5, column=1, sticky='w')
...
def start_work():
    ...
    for i in range(len(target_delta)):
        ax.plot([epsilon_delta[i].spent_eps for epsilon_delta in loss], accuracy,
                 label='delta=%.2g' % target_delta[i], color=delta_color[i])
    ax.set_xlabel('epsilon')
    ax.set_ylabel('accuracy')
    ax.legend()

    plt.savefig('work2type%dsigma%fclip%fbatch%d.jpg' % (sigma_type.get(), sigma, clip_bound, batch_size))
    plt.show()
